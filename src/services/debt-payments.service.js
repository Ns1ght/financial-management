...
export const processDebtPayment = async ({ debt_group_id, payment_date }) => {
  ...
  try {
    ...
    // 2. Fetch all active sub-debts for this group — now reusing the shared helper,
    //    passing our transaction's client so it stays inside this same transaction
    const rawSubDebts = await getActiveSubDebtsByGroupId(debt_group_id, client);
    const activeSubDebts = rawSubDebts.map(d => ({
      id: d.id,
